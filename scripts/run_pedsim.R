...
for( scenario in scenario_names ){    
    ...
    output_folder_name <- paste0( c( '~/neomatrix/team3/ped_sim_outputs', scenario), collapse = '/' )
    dir.create( output_folder_name )
    files <- list.files( input_folder_name )[ c(-1, -((length(files)-3):length(files)) ) ]
    for( file_name in files ){
        ...
        command <- paste0( c(
            "ped-sim -d ~/neomatrix/team3/ped6.def --pois -m ~/neomatrix/team3/temp_gen_map.txt -i",
            full_name,
            '-o',
            output_name,
            '--nogz'), collapse = ' '
